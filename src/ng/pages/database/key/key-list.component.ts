...
export class KeyListComponent extends KeyTypeBase implements OnInit, OnChanges {
    paging: KeyPaging;
    ...
    async appendValue(event: Event): Promise<void> {
        try {
            await this.keyNewOrSetDialog.show({ $event: event, type: 'append', model: { type: 'list', key: this.p3xrKey } });
            this.refreshKey();
    ...
    async editValue(index: number, value: any, event: Event): Promise<void> {
        try {
            ...
            await this.keyNewOrSetDialog.show({
                ...
                model: { type: 'list', key: this.p3xrKey, index, value: editValue },
            });
            this.refreshKey();
        } catch (e) { this.common.generalHandleError(e); }
    }
