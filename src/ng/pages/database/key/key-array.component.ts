import { Component, Inject, OnInit, OnChanges, SimpleChanges, ChangeDetectorRef, CUSTOM_ELEMENTS_SCHEMA, ViewEncapsulation, ChangeDetectionStrategy } from '@angular/core';
...
@Component({
    ...
    templateUrl: './key-array.component.html',
    changeDetection: ChangeDetectionStrategy.Eager,
    encapsulation: ViewEncapsulation.None,
})
export class KeyArrayComponent extends KeyTypeBase implements OnInit, OnChanges {
    paging: KeyPaging;
    ...
    updatePagedItems(): void {
        ...
        const keys = Object.keys(this.p3xrValue).sort((a, b) => Number(a) - Number(b));
        this.pagedItems = keys.slice(this.paging.startIndex, this.paging.endIndex)
            .map(k => ({ key: k, value: this.p3xrValue[k] }));
    }

    async addArray(event: Event): Promise<void> {
        try {
            await this.keyNewOrSetDialog.show({ $event: event, type: 'append', model: { type: 'array', key: this.p3xrKey } });
            this.refreshKey();
        } catch (e) { this.common.generalHandleError(e); }
    }

    async deleteArrayIndex(index: string, event: Event): Promise<void> {
        try {
            await this.common.confirm({ message: this.i18n.strings().confirm?.deleteArrayIndex });
            await this.socket.request({ action: 'key/array-delete-index', payload: { key: this.p3xrKey, index } });
            this.common.toast(this.i18n.strings().status?.deletedArrayIndex);
            this.refreshKey();
        } catch (e) { this.common.generalHandleError(e); }
    }

    async editValue(index: string, value: any, event: Event): Promise<void> {
        try {
            ...
            await this.keyNewOrSetDialog.show({
                ...
                model: { type: 'array', key: this.p3xrKey, index, value: editValue },
            });
            this.refreshKey();
        } catch (e) { this.common.generalHandleError(e); }
    }
    ...
    downloadItem(index: string): void { this.downloadBuffer(this.p3xrValueBuffer[index], `${this.p3xrKey}-${index}`); }
}
