const strings = {
  ...
  label: {
    ...
    aiGroqApiKeyInfo: "Необязательно. Собственный ключ API Groq для лучшей производительности. Получите бесплатный ключ на",
    aiGroqApiKeyPlaceholder: "gsk_...",
    aiGroqApiKeySaved: "Ключ API ИИ сохранён",
    aiGroqApiKeyInvalid: "Недействительный ключ API Groq",
    aiGroqApiKeyNotSet: "Не задан (по умолчанию сервера)",
    ...
    consoleDrawer: {
      toggleTooltip: "Переключить консоль",
      clearTooltip: "Очистить историю консоли",
      closeTooltip: "Закрыть консоль",
      aiSettingsTooltip: "Настройки AI",
      modeRedis: "REDIS",
      modeAi: "AI",
      connectionChipNoDb: opts => `${opts.name}`,
      connectionChipWithDb: opts => `${opts.name} · db ${opts.db}`,
      pageChip: opts => `страница: ${opts.page}`,
      connectingTo: opts => `Подключение к ${opts.name}…`,
      connectedTo: opts => `Подключено к ${opts.name} (Redis ${opts.version} ${opts.mode}, загружено ${opts.modules} модулей)`,
      connectedToNoInfo: opts => `Подключено к ${opts.name}`,
      disconnectedFrom: opts => `Отключено от ${opts.name}`,
      readyIndicator: "Готово.",
    },
    cheatsheet: {
      title: "Шпаргалка AI — о чём можно спросить?",
      subtitle: "Нажмите на любой запрос, чтобы вставить его в консоль. Затем нажмите Enter.",
      searchPlaceholder: "Фильтр запросов…",
      openOfficialDocs: "Команды Redis ↗",
      openOfficialDocsTooltip: "Открыть официальный справочник команд Redis на redis.io",
      closeTooltip: "Закрыть (Esc)",
      empty: "По вашему фильтру запросы не найдены.",
      footerHint: "Подсказка: введите \"ai:\" и дальше что угодно на любом языке — AI понимает 54 языка и при необходимости использует текущее состояние Redis.",

      groups: {
        diagnostics: {
          name: "Живая диагностика",
          description: "Попросите AI исследовать текущее состояние сервера с помощью безопасных инструментов только для чтения.",
          prompts: [
            "почему память высокая?",
            "покажи 10 самых медленных запросов",
            "какие клиенты подключены?",
            "какая политика maxmemory?",
            "были ли недавние вытеснения?",
            "есть ли события латентности?",
            "как долго сервер работает?",
            "какой hit rate?",
            "покажи использование CPU",
            "суммируй keyspace",
            "сколько памяти использует каждый тип данных?",
            "не блокирует ли что-то сервер прямо сейчас?"
          ]
        },
        keys: {
          name: "Ключи",
          description: "Просматривайте, ищите и анализируйте ключи без переходов по дереву.",
          prompts: [
            "найди все ключи, совпадающие с user:*",
            "сколько ключей в каждой базе данных?",
            "покажи самый большой hash в этой базе",
            "найди ключи с TTL меньше 60 секунд",
            "у каких ключей нет TTL?",
            "какой тип у ключа session:abc?",
            "оцени память, используемую префиксом \"session:\"",
            "покажи object encoding ключа user:42",
            "есть ли ключи, которые скоро истекут?",
            "какое пространство имён использует больше всего памяти?"
          ]
        },
        dataTypes: {
          name: "Типы данных",
          description: "Формулировки на естественном языке для создания/чтения/обновления каждого типа Redis.",
          prompts: [
            "создай hash с именем user:1 и полями name=Alice age=30",
            "добавь три элемента в список tasks",
            "добавь участников в set favourites",
            "добавь участников с баллами в sorted set leaderboard",
            "добавь событие в stream events",
        ...
        modules: {
          name: "Модули",
          description: "Запросы по загруженным модулям Redis (категории ниже появляются только при наличии модуля).",
          prompts: []
        },
        json: {
          name: "RedisJSON",
          description: "Доступно, когда загружен модуль ReJSON.",
          prompts: [
            "создай JSON-документ в user:42 с { name: \"Alice\", age: 30 }",
        ...
        search: {
          name: "RediSearch",
          description: "Доступно, когда загружен модуль поиска.",
          prompts: [
            "покажи все full-text индексы",
            "выполни full-text поиск по \"redis\" в индексе idx:products",
            "создай индекс на основе hash с полями title (TEXT) и price (NUMERIC)",
            "покажи информацию об индексе idx:products",
            "удали индекс idx:products",
            "найди документы, где price между 10 и 50",
            "составь гибридный поиск, который объединяет текст и векторное сходство"
          ]
        },
        timeseries: {
          name: "RedisTimeSeries",
          description: "Доступно, когда загружен модуль timeseries.",
          prompts: [
            "покажи все ключи timeseries",
            "добавь точку данных в temp:room1",
        ...
        bloom: {
          ...
          description: "Доступно, когда загружен модуль bf.",
          prompts: [
            "проверь, существует ли элемент foo в Bloom filter spam:ips",
            "добавь элементы в Bloom filter spam:ips",
            "создай Top-K с именем popular и K=10",
            "выполни запрос к count-min sketch traffic для ключа /home",
            "добавь значения в t-digest и получи 95-й процентиль",
            "покажи информацию о Bloom filter spam:ips"
          ]
        },
        vectorSet: {
          name: "VectorSet (Redis 8+)",
          description: "Доступно при обнаружении Redis 8+ (нативный тип VECTORSET).",
          prompts: [
            "добавь вектор в embeddings",
            "найди 10 наиболее похожих векторов для вектора запроса",
            "покажи размерности и количество элементов vectorset embeddings",
            "удали элемент из vectorset embeddings",
        ...
        redis8: {
          ...
          prompts: [
            "установи TTL поля hash с помощью HEXPIRE",
            "получи digest строкового значения",
        ...
        scripting: {
          name: "Скрипты",
          description: "Генерируйте Lua / EVAL-скрипты по описаниям на естественном языке.",
          prompts: [
            "напиши атомарный скрипт, который увеличивает счётчик X только если Y > 5",
            "сгенерируй 100 случайных ключей с помощью Lua",
            "преобразуй этот shell pipeline в один EVAL: keys user:* | GET | grep inactive | DEL",
            "перенеси пакетную операцию в Lua для безопасности в кластере",
            "сделай обновление в стиле check-and-set одним вызовом Lua",
            "пройдись по hash и удали поля, совпадающие с шаблоном"
          ]
        },
        cluster: {
          name: "Кластер",
          description: "Показывается только в режиме кластера.",
          prompts: [
            "покажи информацию о кластере",
            "покажи узлы кластера",
            "покажи 10 лучших слотов по количеству ключей",
            "покажи 10 лучших слотов по памяти",
            "какой master владеет слотом 5000?"
          ]
        },
        acl: {
          name: "ACL (Redis 6+)",
          description: "Просматривайте пользователей контроля доступа и текущее подключение.",
          prompts: [
            "под каким пользователем я подключён?",
            "покажи всех пользователей ACL",
            "какие у меня права?",
            "покажи правила пользователя default"
          ]
        },
        qna: {
          name: "Общие вопросы и ответы",
          description: "Задавайте вопросы о Redis — без инструментов, только ответы.",
          prompts: [
            "что такое ZADD?",
            "как работает failover в кластере?",
            "объясни SCAN и KEYS",
            "когда мне стоит использовать EVAL вместо нескольких команд?",
            "какие есть варианты персистентности в Redis?",
            "в чём разница между RDB и AOF?",
            "как Redis Sentinel выбирает нового master?",
            "объясни hash tags в режиме кластера"
          ]
        },
        translate: {
          name: "Естественный язык → команда Redis",
          description: "Опишите, что хотите сделать, на любом из 54 языков; AI напишет команду Redis.",
          prompts: [
            "удали ключ user:42",
            "переименуй ключ foo в bar",
            "установи истечение ключа session:abc через 10 секунд",
            "скопируй ключ source в destination",
            "увеличь счётчик visits на 5",
            "установи ключ greeting в \"hello\" на 1 час",
            "удали все ключи user:*",
            "покажи мне 10 самых загруженных ключей"
          ]
        }
      }
    },
    ...
    aclAuthHint: "Используйте имя пользователя и пароль Redis ACL для аутентификации. Оставьте пустым для пользователя по умолчанию без пароля.",
  ...
  status: {
    ...
    deletedStreamTimestamp: "Запись потока удалена",
  },
  code: {
  ...
  form: {
    ...
    treeSettings: {
      ...
      keyCount: (opts) => {
        return `Количество ключей: ${opts?.keyCount ?? 0}`;
      },
      label: {
  ...
  page: {
    ...
    monitor: {
      ...
      noSlowQueries: "Медленных запросов не зафиксировано.",
      confirmSlowLogReset: "Вы уверены, что хотите сбросить медленный журнал?",
      slowLogResetDone: "Медленный журнал был сброшен.",
      totalCommands: "Всего",
      expired: "Истекли",
      evicted: "Вытеснено",
      ...
      uptime: "Время работы",
      keyspace: "Ключевое пространство Redis",
      keys: "Ключи Redis",
      expires: "Срок действия истекает",
      noKeyspace: "Нет ключей",
      persistence: "Сохранение данных",
      rdbLastSave: "Последнее сохранение RDB",
      rdbStatus: "Статус РБД",
      rdbChanges: "Изменения с момента последнего сохранения",
      aofEnabled: "АОФ включен",
      aofSize: "Размер АОФ",
      replication: "Репликация Redis",
      role: "Роль репликации",
      replicas: "Подключенные реплики",
      masterHost: "Основной хост",
      linkStatus: "Статус канала репликации",
      cpu: "Использование ЦП",
      cpuSys: "Система",
      cpuUser: "Пользователь",
      modules: "Загруженные модули Redis",
      noModules: "Модули Redis не загружены.",
      clusterSlotMap: "Карта слотов кластера Redis",
      slotRange: "Диапазон слотов кластера",
      totalSlots: "Всего слотов кластера",
      noClusterData: "Данные кластера Redis недоступны.",
    },
    analysis: {
      title: "Анализ памяти",
      ...
      prefixMemory: "Память по префиксу",
      topKeysByMemory: "Крупнейшие ключи по памяти",
      expirationOverview: "Срок действия ключей",
      memoryBreakdown: "Разбивка памяти",
      keysScanned: "Просканировано ключей",
      totalMemory: "Общая память",
      rssMemory: "RSS память",
      peakMemory: "Пиковая память",
      luaMemory: "Lua память",
      overheadMemory: "Накладные расходы",
      datasetMemory: "Набор данных",
      fragmentation: "Фрагментация",
      allocator: "Аллокатор",
      withTTL: "С TTL",
      ...
      doctorNoData: "Нажмите Обновить, чтобы запустить диагностику Memory Doctor.",
    },
    acl: {
      title: "Пользователи ACL",
      ...
      disabled: "Отключено",
      commands: "Команды",
      commandsHint: "например, +@all or +@read -@dangerous",
      keys: "Ключевые шаблоны Redis",
      keysHint: "например, ~* or ~user:*",
      channels: "Каналы Pub/Sub",
      channelsHint: "например, &* or &notifications:*",
      password: "Пароль",
      noPassword: "Нет пароля (nopass)",
      passwordHint: "Оставьте пустым, чтобы сохранить текущий пароль",
      currentUser: "Текущий",
      createUser: "Создать пользователя",
      editUser: "Изменить пользователя",
      deleteUser: "Удалить",
      confirmDelete: "Вы уверены, что хотите удалить пользователя ACL?",
      userDeleted: "Пользователь ACL был удален.",
      userSaved: "Пользователь ACL был сохранен.",
      cannotDeleteDefault: "Невозможно удалить пользователя по умолчанию.",
      cannotDeleteSelf: "Невозможно удалить текущего подключенного пользователя.",
      noUsers: "ACL требует Redis 6.0+.",
      groupCommon: "Общие",
      groupDataTypes: "Типы данных",
      groupOperations: "Операции",
      rules: "Правила",
      rulesHint: "Токены, разделенные пробелами (например, on >password +@all ~* &*).",
      defaultUserWarning: "Внимание: изменение пользователя по умолчанию может заблокировать все соединения. Если это произойдет, вам придется перезапустить Redis или использовать redis-cli для восстановления доступа.",
    },
    overview: {
    ...
    key: {
      label: {
        ...
        encoding: "Кодировка",
        compression: "Сжатие",
        aiRateLimited: "Достигнут лимит запросов AI. Попробуйте позже или используйте собственный ключ Groq API в Настройках.",
        aiError: "Запрос AI не удался",
        length: "Размер",
        ttl: "TTL",
    ...
    treeControls: {
      ...
      search: {
        ...
        info: (opts) => "Поиск в клиенте означает совпадание с содержимым поля поиска. Серверный поиск означает поиск паттерном на подобие *{искомый-текст}*. Для поиска больших наборов лучше использовать серверный поиск. Для меньших поисковых наборов лучше использовать поиск клиентом." + ` Если количество ключей превышает ${opts?.maxLightKeysCount ?? 110000}, можно будет искать только серверным поиском.`,
  ...
  promo: {
    ...
    description: "Откройте для себя нашего бесплатного сетевого AI-ассистента на network.corifeus.com — анализируйте домены, IP-адреса, DNS-записи, SSL-сертификаты, безопасность электронной почты и сетевую инфраструктуру. Работает на базе AI для мгновенных и полных результатов.",
    disclaimer: "Эта промоакция показывается только на demo-сайте и не будет отображаться в развертываниях Docker, Electron или веб-приложения.",
    toastMessage: "Попробуйте нашего бесплатного сетевого AI-ассистента на network.corifeus.com — анализируйте домены, DNS, SSL и многое другое!",
    visit: "Открыть network.corifeus.com",
  },
  promoMeeting: {
    title: "Meet Assistant SaaS",
    description: "Полный рабочий процесс встреч для европейского бизнеса на meeting.corifeus.com — живой перевод на 21 язык, AI-резюме, задачи, решения, участники и ключевые цитаты создаются автоматически. Пользовательский словарь, библиотека встреч с поиском, ссылки только для чтения, которыми можно делиться, сводка по email в один клик. Биллинг EU — Stripe Tax, соответствие VAT, цены в EUR (Solo €19.99 / Pro €39.99 / Business €99.99 в месяц, без привязки). По умолчанию соответствует GDPR — без файлов cookie для отслеживания, зашифрованное хранилище.",
    disclaimer: "Эта промоакция показывается только на demo-сайте и не будет отображаться в развертываниях Docker, Electron или веб-приложения.",
    toastMessage: "Попробуйте Meet Assistant SaaS на meeting.corifeus.com — живой перевод на 21 язык, AI-резюме, задачи. 1 минута бесплатно, без регистрации, соответствует GDPR.",
    visit: "Посетить meeting.corifeus.com",
  }
};
