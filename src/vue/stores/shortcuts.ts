...
import { emitCommand } from './main-command.store'
...
function isConnected(): boolean {
    return !!useRedisStateStore().connection
}
...
function requireConnection(action: () => void) {
    if (isConnected()) {
        ...
        const strings = useI18nStore().strings
        useCommonStore().toast(strings?.label?.connectFirst)
    }
}
...
function requireConnectionAndHome(action: () => void) {
    ...
    if (!location.pathname.startsWith('/database') && !location.pathname.startsWith('/vue/database')) {
...
const shortcuts: ShortcutDef[] = isElectron ? [
    {
        ...
        action: () => requireConnection(() => {
            useMainCommandStore().refresh()
        }),
    },
    {
        key: 'F5', label: 'F5',
        descriptionKey: 'shortcutRefresh',
        action: () => requireConnection(() => useMainCommandStore().refresh()),
    },
    {
    ...
    },
    {
        ...
        action: () => {
            useCommonStore().setCommandPaletteOpen(true)
        },
    },
    {
        key: 'd', ctrlKey: true, label: 'Ctrl+D',
        descriptionKey: 'shortcutDisconnect',
        action: () => requireConnection(() => useMainCommandStore().disconnect()),
    },
] : []
