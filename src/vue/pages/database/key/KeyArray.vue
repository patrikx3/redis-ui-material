...
/**
 * KeyArray — Redis 8.8 sparse array key type renderer.
 * Server shapes the value like a hash: an { [index: string]: value } object.
 * Rendered as an (index → value) table, indices sorted ascending numerically.
 * No per-element TTL (unlike hash). Add/delete/edit/copy/json/download per row.
 */
...
const pagedItems = ref<Array<{ index: string; value: any }>>([])
...
function getKeys(): string[] {
    ...
    // Array indices are numeric strings — keep ascending numeric order (sparse-friendly)
    return Object.keys(props.value).sort((a, b) => Number(a) - Number(b))
}

function updatePaged(p: Paging) {
    paging.value = p
    const keys = getKeys()
    pagedItems.value = keys.slice(p.startIndex, p.endIndex).map(k => ({ index: k, value: props.value[k] }))
}
watch(() => props.value, () => updatePaged(rePaging(paging.value, getKeys().length)), { immediate: true })

function addItem() { editDialogData.value = { type: 'append', model: { type: 'array', key: props.keyName } }; editDialogOpen.value = true }
function editItem(index: string, val: any) { editDialogData.value = { type: 'edit', model: { type: 'array', key: props.keyName, index, value: val } }; editDialogOpen.value = true }
...
async function deleteIndex(index: string) {
    try { await common.confirm({ message: str(strings.value?.confirm?.deleteArrayIndex) }); await request({ action: 'key/array-delete-index', payload: { key: props.keyName, index } }); common.toast(str(strings.value?.status?.deletedArrayIndex)); emit('refresh') }
...
<template>
    <div>
        ...
        <div class="p3xr-key-table-header">
            <span style="flex:20%;">{{ str(strings?.page?.key?.array?.table?.index) }}</span>
            <span style="flex:60%;">{{ str(strings?.page?.key?.array?.table?.value) }}</span>
        ...
        <div v-for="(item, i) in pagedItems" :key="item.index" class="p3xr-key-table-row" :class="{ 'p3xr-key-table-odd': i % 2 === 0 }">
            ...
            <span class="p3xr-key-table-actions">
                <v-tooltip v-if="!isReadonly" :text="str(strings?.intention?.delete)" location="top"><template #activator="{ props: tp }"><v-icon v-bind="tp" size="24" class="p3xr-key-icon" style="color:rgb(var(--v-theme-error));" @click="deleteIndex(item.index)">mdi-delete</v-icon></template></v-tooltip>
                ...
                <v-tooltip :text="str(strings?.intention?.downloadBuffer)" location="top"><template #activator="{ props: tp }"><v-icon v-bind="tp" size="24" class="p3xr-key-icon" style="color:rgb(var(--v-theme-secondary));" @click="downloadBuffer(valueBuffer?.[item.index], keyName, `${keyName}-${item.index}`)">mdi-download</v-icon></template></v-tooltip>
